...
function saveToHistory() {
  ...
  // Add the content to the document history array
...
document.getElementById('previous-button').addEventListener('click', function() {
  ...
  // If the index is out of bounds, set it to the last element in the history array
  if (currentIndex < 0) {
    currentIndex = documentHistory.length - 1;
  }

  // Set the editor content to the version at the current index
  CKEDITOR.instances.editor.setData(documentHistory[currentIndex]);
});
...
document.getElementById('next-button').addEventListener('click', function() {
  ...
  // If the index is out of bounds, set it to the first element in the history array
  if (currentIndex >= documentHistory.length) {
    currentIndex = 0;
  }
...
document.getElementById('export-button').addEventListener('click', function() {
    ...
    // Convert the HTML content to plain text and remove the HTML tags
    var converter = new showdown.Converter();
    editorContent = converter.makeHtml(editorContent);
